...
function websiteTitle($title=null){
  ...
  $text2=$text&&$text1?' - ':'';
  if($title){
    return $title.$hasTitle.$text;
  }else{
    return $text.$text2.$text1;
  }
...
function myCart($cookie=null){
  ...
  if( $cookie)
  {
    ...
    $carts = Cart::where('cookie', $cookie)->latest()->select(['id','user_id','product_id','sku_id','color','size','cookie','quantity','emi','coupon_id','warranty_charge','warranty_note'])->get();
